  ...
  expect_equal(
    ...
    structure(
      list(
        ...
        avg = c(
          8.06452230500802, 6.12987251658832, 4.7549349140692, 1.77414578587699
        )
      ),
...
test_that("*mgtdhid* produces wrong results", {
  ...
  expect_equal(
    mgtdhid(
      id = c(22217L, 26094L, 30433L),
      tau = as.POSIXct("2023-04-11", tz = "UTC"), depth = 2.4
    ),
    c(0.929297595932886, 3.45956953933863, -0.192953283493775)
  )
})
...
test_that("*mgtdhidt* produces wrong results", {
  ...
  expect_equal(
    ...
    mgtdhidt(
      tau = as.POSIXct(
        seq.int(1672531200, 1703980800, 3600), tz = "UTC", origin = "1970-01-01"
      ),
      depth = d24
    )
  )
...
test_that("*mgtdhgeot* produces wrong results", {
  ...
  expect_equal(
    c(
      mgtdhgeot(
        tau = 1440L, lat = lat[["s28434"]], lon = lon[["s28434"]], depth = d24
      ),
      mgtdhgeot(
        tau = 1440L, lat = lat[["s28418"]], lon = lon[["s28418"]], depth = d24
      ),
      mgtdhgeot(
        tau = 1440L, lat = lat[["s23711"]], lon = lon[["s23711"]], depth = d24
      )
    ),
    mgtdhid(
      id = c(28434L, 28418L, 23711L),
      tau = as.POSIXct("2023-03-02", tz = "UTC"),
      depth = d24
    )
  )
...
test_that(
  paste(
    "*mgtdhgeot* and others produce wrong results",
    "without execution parallelization"
  ), {
  lat <- c(
    s28434 = 56.65, s28418 = 56.47, s23711 = 62.70, CP1 = 57, CP2 = 56.00
  )
  lon <- c(
    s28434 = 57.78, s28418 = 53.73, s23711 = 56.20, CP1 = 57, CP2 = 57.00
  )

  # Google maps distances from CP2 to the nearest stations, [m]
  r <- c(
    s28434 = 86986.91, s28418 = 209396.94, s28630 = 192034.70
  )
  d24 <- 2.4
  ...
  expect_equal(
    ...
    mgtdhid(
      id = c(28434L, 28418L, 23711L),
      tau = as.POSIXct("2023-03-02", tz = "UTC"), depth = d24
    )
  )
  ...
  expect_equal(
    mgtdhgeot(
      tau = c(1440L, 1450L, 1540L), lat[["CP2"]], lon[["CP2"]], depth = d24
    ),
    c(
      mgtdhgeo(
        lat[["CP2"]], lon[["CP2"]],
      ...
      mgtdhgeo(
        lat[["CP2"]], lon[["CP2"]],
        tau = as.POSIXct("2023-03-02", tz = "UTC") + 3600 * 10, depth = d24
      ),
      mgtdhgeo(
        lat[["CP2"]], lon[["CP2"]],
        tau = as.POSIXct("2023-03-02", tz = "UTC") + 3600 * 100, depth = d24
      )
    )
  )
  rm(d24, r, lon, lat)
...
test_that(
  ...
  expect_equal(
    mgtdhgeo(
      head(lat, 3), head(lon, 3), tau = 1440L, depth = d24,
  ...
  expect_equal(
    mgtdhgeo(
      lat[["CP2"]], lon[["CP2"]], tau = 1440L, depth = d24,
      use_cluster = !nzchar(Sys.getenv("_R_CHECK_LIMIT_CORES_", ""))
    ),
    drop(
      mgtdhid(
  ...
  expect_equal(
    mgtdhgeot(tau = 1440L, lat[["CP2"]], lon[["CP2"]], depth = d24),
    mgtdhgeo(
      lat[["CP2"]], lon[["CP2"]], tau = as.POSIXct("2023-03-02", tz = "UTC"),
      depth = d24,
      use_cluster = !nzchar(Sys.getenv("_R_CHECK_LIMIT_CORES_", ""))
    )
  )
