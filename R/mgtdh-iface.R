...
#'  geographically dependant parameters which values were established for each
...
#'
#' @param depth
#'  depth at which the ground temperature is calculated, [\emph{m}].
...
#'
#' @seealso
#'  \code{\link{geodist}} and \code{\link{geoarea}} for calculating geographical
#'  metrics.
#'
#' @examples
...
mgtdhid <- function(id, tau = 1440L, depth = 2.4) {
  ...
  checkmate::assert_integer(
    id,
    lower = 0, upper = 1e5, any.missing = FALSE, min.len = 1
  )
  if (checkmate::test_posixct(tau, any.missing = FALSE, len = 1)) {
    tau <- as.POSIXct(tau, tz = L_TIMEZONE)
    tau <- (
      as.integer(tau) -
      as.integer(as.POSIXct(format(tau, L_YEAR_START), tz = L_TIMEZONE))
    ) %/% L_HOUR
  }
  checkmate::assert_integer(
    tau,
    lower = 0, upper = 8784, any.missing = FALSE, len = 1
  )
  checkmate::assert_double(
    depth,
    lower = 0.3, upper = 10.0, any.missing = FALSE, len = 1
  )
  ...
  mgtdh(
    x     = tau,          depth       = depth,
    avg   = s[["avg"]],   ampl1       = s[["ampl1"]],
    ampl2 = s[["ampl2"]], pl1         = s[["pl1"]],
    pl2   = s[["pl2"]],   diffusivity = s[["diffusivity"]]
  )
}
...
mgtdhidt <- function(tau, id = 28434L, depth = 2.4) {
  ...
  checkmate::assert_integer(
    tau,
    lower = 0, upper = 8784, any.missing = FALSE, min.len = 1
  )
  checkmate::assert_integer(
    id,
    lower = 0, upper = 1e5, any.missing = FALSE, len = 1
  )
  checkmate::assert_double(
    depth,
    lower = 0.3, upper = 10.0, any.missing = FALSE, len = 1
...
mgtdhgeo <- function(lat, lon, tau = 1440L, depth = 2.4, use_cluster = FALSE) {
  ...
  checkmate::assert_double(
    lat,
    lower = 42, upper = 68, any.missing = FALSE, min.len = 1
  )
  checkmate::assert_double(
    lon,
    lower = 30, upper = 166, any.missing = FALSE, min.len = 1
  )
  ...
  if (use_cluster) {
    ...
    stream <- parallel::clusterMap(
      cluster,
      function(x, y) {
        pipenostics::mgtdhgeot(tau = tau, lat = x, lon = y, depth = depth)
      },
      lat, lon,
...
mgtdhgeot <- function(tau, lat = 57, lon = 57,  depth = 2.4) {
  ...
  checkmate::assert_double(
    lat,
    lower = 42, upper = 68, any.missing = FALSE, len = 1
  )
  checkmate::assert_double(
    lon,
    lower = 30, upper = 166, any.missing = FALSE, len = 1
  )
