...
#'
#' @param mu
#'  dynamic viscosity of fluid in pipe, [\emph{kg/m/s}].
#'  Type: \code{\link[checkmate]{assert_double}}.
#'
#' @param u
#'  mean velocity of fluid in pipe, [\emph{m/s}].
#'  Type: \code{\link[checkmate]{assert_double}}.
#'
#' @param rho
#'  mass density of fluid in pipe, [\emph{kg/m³}].
#'  Type: \code{\link[checkmate]{assert_double}}.
#'
#' @param v
#'  volumetric flow rate of fluid in pipe, [\emph{m³/s}].
#'  Type: \code{\link[checkmate]{assert_double}}.
#'
#' @param m
#'  mass flow rate of fluid in pipe, [\emph{kg/s}].
#'  Type: \code{\link[checkmate]{assert_double}}.
#'
#' @return
...
re_u <- function(d, mu, u, rho) {
  checkmate::assert_double(
    d,
    lower = 10e-3, upper = 2, any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    mu,
    lower =  1.0e-8, upper = 2, any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    rho,
    lower =  0.0, upper = 2000, any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    u,
    lower =  0.0, upper = 10, any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_true(commensurable(c(
    length(d), length(mu), length(u), length(rho)
  )))

  d / mu * rho * u  ## [m] / [kg/m/s] * [kg/m³] * [m/s] = []
}
...
re_v <- function(d, mu, v, rho) {
  ...
  checkmate::assert_double(
    v,
    lower =  0.0, upper =    5, any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    rho,
    lower =  0.0, upper = 2000, any.missing = FALSE, min.len = 1L
  ...
  a <- .25 * pi * d^2  ## [m²]
  d / mu / a * rho * v  ## [m]/[kg/m/s]/[m²]*[kg/m³]*[m³/s] = []
}
