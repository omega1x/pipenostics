...
#'}
#'
#' # Raw unit tests used for debugging  ----
...
#' )
#'
#' with(subset(nx, acceptor == 7),
#'      stopifnot(
#'        temperature +
#'          m325dropt(
#'            temperature, pressure, consumption,
#'            d, len, year, insulation, laying, beta, exp5k
#'          ) == subset(output, node == 6 & trace == 7 & aggregation == "identity",
#'                      "temperature"),
#'        pressure +
#'          pipenostics::dropp(
#'            temperature, pressure, consumption,
...
#' )
#'
#' with(subset(nx, acceptor == 15),
#'      stopifnot(
#'        temperature +
#'          m325dropt(
#'            temperature, pressure, consumption,
#'            d, len, year, insulation, laying, beta, exp5k
#'          ) == subset(output, node == 14 & aggregation == "identity", "temperature"),
#'        pressure +
#'          pipenostics::dropp(
#'            temperature, pressure, consumption,
#'            d*1e-3, len, roughness, inlet, outlet, "romeo"
#'          )  == subset(output, node == 14 & aggregation == "identity",
#'                       "pressure")
#'      )
#' )
#'
#' with(subset(nx, acceptor == 24),
#'      stopifnot(
#'        temperature +
#'          m325dropt(
#'            temperature, pressure, consumption,
#'            d, len, year, insulation, laying, beta, exp5k
#'          ) == subset(output, node == 22 & trace == 24 & aggregation == "identity",
#'                      "temperature"),
#'        pressure +
#'          pipenostics::dropp(
#'            temperature, pressure, consumption,
...
#' )
#'
#' with(subset(nx, acceptor == 26),
#'      stopifnot(
#'        temperature +
#'          m325dropt(
#'            temperature, pressure, consumption,
#'            d, len, year, insulation, laying, beta, exp5k
#'          ) == subset(output, node == 23 & aggregation == "identity",
#'                      "temperature"),
#'        pressure +
#'          pipenostics::dropp(
#'            temperature, pressure, consumption,
...
#' )
#'
#' # * Tests for mass balance ----
#' stopifnot(
#'   subset(
#'     output, node == 6 & aggregation == "median", "consumption"
#'   ) == colSums(subset(nx, acceptor %in% c(3, 7, 9), "consumption"))
#' )
#'
#' stopifnot(
#'   subset(
#'     output, node == 20 & aggregation == "median", "consumption"
#'   ) == colSums(subset(nx, acceptor %in% c(19, 21, 26), "consumption"))
#' )
#'
#' stopifnot(
#'   subset(
#'     output, node == 13 & aggregation == "median", "consumption"
#'   ) == colSums(subset(nx, !is.na(consumption), "consumption"))
#' )
#'
#'
#' @export
