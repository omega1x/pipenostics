...
#'  supplying underground pipe as a function of construction, operation,
...
#'   Type: \code{\link[checkmate]{assert_double}}.
#' @param lambda0
#'   thermal conductivity of environment, [\emph{W/m/°C}]. For underground pipe
...
#'   \code{duration}, [\emph{kcal}]. If \code{len} of pipe is 1 \emph{m} (meter)
#'  as well as \code{duration} is set to 1 \emph{h} (hour) (default values) then
#'  the return value is also the \emph{specific heat loss power},
#'  [\emph{kcal/m/h}] and so comparable with those prescribed by
...
m278hlund <- function(
  ...
  lambda1 = 0.09, lambda2 = 0.07, k1 = 1, k2 = k1, lambda0 = 1.74, z = 2,
  s = 0.55, len = 1, duration = 1
) {
  ...
  checkmate::assert_double(
    lambda0,
    lower = 1e-3, upper = 3, finite = TRUE, any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    z,
    lower = .1, upper = 10, finite = TRUE, any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    s,
    lower = .5 * (d1 + d2), upper = 10, finite = TRUE, any.missing = FALSE,
    min.len = 1L
  )
  checkmate::assert_double(
    len,
  ...
  q1 <- {
    ((t1 - t0) * (R2_ins + R2_soil) -
    (t2 - t0) * R12) / ((R1_ins + R1_soil) * (R2_ins + R2_soil) - R12^2)
  }
  q2 <- {
    ((t2 - t0) * (R1_ins + R1_soil) -
    (t1 - t0) * R12) / ((R1_ins + R1_soil) * (R2_ins + R2_soil) - R12^2)
  }
