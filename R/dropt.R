...
dropt <- function(
  ...
  flow_rate   =        250, # [ton/h]
  loss_power  =       7000  # [kcal/h]
) {

  checkmate::assert_double(
    temperature,
    lower = 0, upper = 350, finite = TRUE,  any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    pressure,
    lower = 8.4e-2, upper = 100, finite = TRUE, any.missing = FALSE,
    min.len = 1L
  )
  checkmate::assert_double(
    flow_rate,
    lower = 1e-3, upper = 1e5, finite = TRUE, any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    loss_power,
  ...
  loss_power_J <- loss_power / JOULE  # [kJ/h]
  g <- flow_rate * 1e3                # [kg/h]

  loss_power_J / g / unname(
    iapws::if97(what = "cp", t = pipenostics::k_c(temperature), pressure)[, 1]
  )  # [°C]=[°K]
}
