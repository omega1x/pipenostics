...
#' library(pipenostics)
#'
#' # Corrosion is low, no limit for the corroded area length:
#' b31gacl(1093, 910, 30, .438, .1, 7.5)
#'
#' # Finite allowed length of the corroded area
...
b31gacl <- function(dep, maop, d, wth, depth, l) {
  ...
  AP[cnd] <- {
    1e-3 * trunc(
      1e3 * sqrt(
        (J[cnd] / (1.0 - 1.1 * dep[cnd] * (1.0 - J[cnd]) / maop[cnd]))^2 - 1.0
      ) + .5
    )
  }
  x[cnd] <- trunc(
    1.1 * dep[cnd] * (1.0 - J[cnd]) /
    (1.0 - 0.6666667 * depth[cnd] / sqrt(wth[cnd] * AP[cnd]^2 + 1.0)) + .5
  )
  cnd2 <- cnd & (x > dep)
  x[cnd2] <- dep[cnd2]
