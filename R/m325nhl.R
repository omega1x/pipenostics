...
#'  }
#'  Type: \code{\link[checkmate]{assert_integer}} and
...
#'
#' @return
#'   Normative heat loss of cylindrical pipe during \code{duration},
#'   [\emph{kcal}]. If \code{len} of pipe is 1 \emph{m} (meter) as well as
#'   \code{duration} is set to 1 \emph{h} (hour) (default values) then the
#'   return value is also the \emph{specific heat loss power},
#'   [\emph{kcal/m/h}], prescribed by
#'   \href{https://docs.cntd.ru/document/902148459}{Minenergo Order 325}.
#'   Type: \code{\link[checkmate]{assert_double}}.
#'
#' @details
...
#'   \href{https://docs.cntd.ru/document/902148459}{Minenergo Order 325}.
#'   Nevertheless, one could control the extrapolation behaviour by \code{extra}
#'   argument: use lower values of \code{extra} for soft curvature near
...
#'
#'  print(fittings_qs)
#'  stopifnot(all(round(fittings_qs, 1)  == c(272.0, 312.8)))
#'
#' @export
m325nhl <- function(
  year = 1986, laying = "underground", exp5k = TRUE, insulation = 0, d = 700,
  temperature = 110, len = 1, duration = 1, beta = FALSE, extra = 2
) {
  norms <- pipenostics::m325nhldata
  checkmate::assert_integerish(
    year,
    lower = 1900L, upper = max(norms[["epoch"]]), any.missing = FALSE,
    min.len = 1L
  )
  ...
  checkmate::assert_double(
    temperature,
    lower = 0, upper = max(norms[["temperature"]]), finite = TRUE,
    any.missing = FALSE, min.len = 1L
  )
  checkmate::assert_double(
    len,
  ...
  checkmate::assert_true(commensurable(c(
    length(year), length(laying), length(exp5k), length(insulation),
    length(d), length(temperature), length(len), length(duration),
    length(beta)
  )))
  checkmate::assert_choice(extra, c(2, 3, 4))

  worker <- function(
    year_value, laying_value, exp5k_value, insulation_value, d_value,
    temperature_value, len_value, duration_value, beta_value
  ) {
    ...
    neighbor_diameter <- with(list(norms_diameter = unique(norms[["d"]])), {
      checkmate::assert_double(
        d_value,
        lower = min(norms_diameter), upper = max(norms_diameter),
    ...
    unit_loss <- vapply(
      neighbor_diameter, function(x, t_carrier) {
        if (is.na(x))
          return(NA_real_)
        with(norms[norms[["d"]] == x, c("temperature", "loss")], {
          zone <- findInterval(
            t_carrier, range(temperature), rightmost.closed = TRUE
          )
          if (zone == 1L) {
            stats::approx(x = temperature, y = loss, xout = t_carrier)[["y"]]
          } else {
